...
DOCKERMAKE_ADMISSION_FILE_PATH=$(pwd)/docker-make-admission.yaml

echo "*** Cloning"

DEST_PATH="${GOPATH}/src/github.com/gardener/gardener-extension-networking-cilium"
rm -rf "${DEST_PATH}"
mkdir -p "${DEST_PATH}"
git clone -b "${BUILD_BRANCH}" https://github.com/metal-stack/gardener-extension-networking-cilium.git "${DEST_PATH}"
